...
type PropType = {
  ...
  /** Optional custom content per slide. If not provided, shows slide index + 1. */
  renderSlide?: (index: number) => React.ReactNode;
  /** Optional class for the root wrapper (e.g. to override --slide-spacing, --slide-size). */
  className?: string;
};
...
function MotionCarousel(props: PropType) {
  const { slides, options, renderSlide, className } = props;
  ...
  return (
    <div className={`w-full space-y-4 [--slide-height:9rem] sm:[--slide-height:13rem] md:[--slide-height:18rem] [--slide-spacing:1.5rem] [--slide-size:55%] ${className ?? ''}`}>
          ...
          {slides.map((index) => {
            ...
            return (
              <motion.div
                ...
                style={!renderSlide ? { height: 'var(--slide-height)' } : undefined}
              >
                <motion.div
                  className={
                    renderSlide
                      ? 'w-full flex items-center justify-center select-none rounded-[28px] overflow-visible'
                ...
                >
                  {renderSlide ? renderSlide(index) : index + 1}
                </motion.div>
              </motion.div>
            );
          })}
